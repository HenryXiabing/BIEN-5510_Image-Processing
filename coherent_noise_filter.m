% coherent_noise_filter.m -  Remove Coherent noise from image  
...
radius = 5; % radius of mask value is empirial
H1 = double(circles2mask( [ double(C1); double(C2)], radius, [yres xres]));
H2 = 1 - H1;

figure;
imagesc(H2);
title('2D Ideal Notch Filter'); drawnow;
...
figure();
mesh(H2(y1:y2, x1:x2)); 
title("Zoomed Magnitude of the circular mask filter"); drawnow;
...
title("Filtered image using Ideal notch filter");drawnow;
